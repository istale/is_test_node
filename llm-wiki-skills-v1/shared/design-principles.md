...
## 2. The Five Skills and One Script

| Component | Type | Reads | Writes | Purpose |
|-----------|------|-------|--------|---------|
| `hermes-llm-wiki` | Skill (Writer) | raw + working | working | Convert raw material into wiki pages |
| `wiki-coverage-audit` | Skill (Follower) | raw + working | patch queue | Check raw key points are covered |
...
| `wiki-compactor` | Skill (Structural) | working | canonical | Restructure working into clean canonical |
| `patch-applier` | Script (deterministic) | patch queue | working | Apply approved patches |
...
## 9. Divergences from nvk/llm-wiki

...
description: Audit whether the wiki working/ layer covers the important content from raw sources AND whether pages expected by the domain schema exist. Use this skill when the user wants to check coverage gaps, verify that raw material has been fully compiled into the wiki, find missing key points, check whether expected-pages are actually present, or run a completeness audit. Also trigger when the user says "audit the wiki", "check coverage", "what did we miss from source X", "is the wiki complete on topic Y", "which expected pages are missing", or "did the writer skip anything important". This skill produces PATCHES to patches/pending/content/ — it never modifies wiki pages directly. Schema-level drift (schema itself is wrong or outdated) belongs to schema-drift-audit, not here. It is the Follower auditor (constructive: adds missing content) — separate from the Interpreter (wiki-relation-detect) and the Evaluator (wiki-contradiction-check).
---
...
Two sub-checks in this pass:

**1a: Raw-to-wiki coverage.** Find things raw literally states that working/ does not contain. Examples:
...
**1b: Expected-pages coverage.** For each entry in `domain-schema/expected-pages.md` marked as `[x]` (should exist):
- Verify the page exists in `working/`
- Verify the page has non-trivial content (not just frontmatter)
- Verify required sections from `domain-schema/kinds/<kind>.md` are present

If a page is marked expected but missing or trivially empty: emit a coverage patch flagging this. Target is `patches/pending/content/`, but the patch does not create the page (that's writer's job) — it flags for reviewer to trigger a writer run.

Also check the reverse: pages in `working/` that are NOT in `expected-pages.md`. This is not necessarily a bug — pages can be legitimately emergent from raw. But note them in the audit report under "Notes for schema-drift-audit" so schema-drift can decide whether to promote them into expected-pages.

**Do NOT modify domain-schema/ from this skill.** If you notice schema itself seems wrong (e.g., a required section is consistently unfillable from raw across many pages), refer it to schema-drift-audit — do not emit schema patches yourself.

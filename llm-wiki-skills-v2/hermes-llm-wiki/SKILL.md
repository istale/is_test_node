...
description: Compile raw source material into structured wiki pages in the working/ layer, following the domain schema. Use this skill whenever the user wants to ingest new raw material into the wiki, create or update wiki pages from meeting notes, decisions, interviews, design documents, or any source that should be reflected in the wiki. Also use when the user says "add this to the wiki", "compile this source", "update the wiki with", or "make a wiki page for". This is the wiki writer — it reads domain-schema/ for page structure, does not audit, does not check for contradictions, and does not restructure existing pages. Those are separate skills (wiki-coverage-audit, wiki-contradiction-check, wiki-compactor, schema-drift-audit). It MAY propose schema patches when raw introduces new page kinds or expected pages.
---
...
3. The current state of `domain-schema/` (readable)
4. The current `source_map.json`

## Workflow

### Step 0: Read the domain schema

Load `domain-schema/kinds/*.md` and `domain-schema/expected-pages.md`. Understand:
- What kinds of pages exist (concept, entity, decision, project, ...) and what sections they should have
- What pages the domain expects to have (checked/unchecked)
- What relations are typical for each kind (from `expected-relations.md`)

Schema tells you STRUCTURE, not FACTS. Raw remains the fact authority.
...
### Step 2: Check existing coverage AND schema expectations

For each entity/concept/decision/project you identified:

1. Check `source_map.json` and `working/` for existing pages
2. Check `domain-schema/expected-pages.md` to see if this page is expected
3. Categorize:
   - **Exists in working, listed in expected-pages** → emit an update patch (Step 4); check off in expected-pages
   - **Exists in working, NOT in expected-pages** → emit an update patch; propose a schema patch to add it to expected-pages (Step 4a)
   - **Not in working, listed in expected-pages** → draft a new page (Step 3); mark checked
   - **Not in working, NOT in expected-pages** → draft a new page (Step 3); propose a schema patch to add it to expected-pages (Step 4a)

If the raw material describes something that doesn't fit any existing kind in `domain-schema/kinds/` (e.g., raw introduces "SOP" as a new kind of artifact and only concept/entity/decision/project kinds exist), STOP and emit a schema patch proposing the new kind. Do NOT invent an off-schema page.
...
**Do not apply your own patches.** Emit them to `patches/pending/content/` and let the reviewer approve. The `patch-applier` script will apply approved patches.

### Step 4a: Emit schema patches when raw introduces new expected pages or kinds

When you identify content that requires updating the schema:

**New expected page** (raw introduces a concept/entity/decision/project not in `expected-pages.md`):

Emit `patch_type: "schema"` patch to `patches/pending/schema/`:
- `target_page`: `"expected-pages.md"`
- `source_skill`: `"hermes-llm-wiki"`
- `pass`: `"explicit"`, `confidence`: `"HIGH"` (raw literally introduces the entity)
- `raw_citations`: cite the raw passage that introduces it
- `wiki_citations`: cite `domain-schema/expected-pages.md` section where it should be added
- `evidence_chain`: 1 step is fine — "raw introduces X; expected-pages does not list it"

**New page kind** (raw introduces content that doesn't fit existing kinds):

STOP writing the corresponding page. Emit a schema patch to `patches/pending/schema/`:
- `target_page`: `"kinds/<proposed-kind>.md"` (proposing new file) OR `"kinds/README.md"` (proposing to document new kind)
- `operation`: create-file for new kind file, OR insert into an index
- Include a proposed structure (required_sections, typical_relations)
- Reviewer must approve before writer can create pages of this kind

Wait for reviewer approval before creating any content page of the proposed kind. This is the one place the writer BLOCKS on human input.
...
- **Follow schema for structure.** Use the section list from `domain-schema/kinds/<kind>.md`. Required sections must be present; recommended sections should be present when raw supports them.
...
## Schema-partial pages

If raw does not support a required section (e.g., schema says every concept page has "## Failure Modes" but raw is a pure definition source with no failure discussion):

- Include the section heading anyway
- Content: `*(No failure modes discussed in current raw sources — flagged as schema-partial.)*`
- Add page frontmatter: `schema_status: partial`
- Note in the compilation report so coverage-audit knows to look

This is honest: the page shows what raw actually contains, and the gap is visible instead of hidden.

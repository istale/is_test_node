...
### Step 1: Load principles + wiki structure + schema

Load `shared/design-principles.md` and `shared/wiki-structure.md`. Confirm canonical/ layout and archive/ retention conventions.

Load `domain-schema/kinds/*.md` to understand target structure per kind. Canonical pages should follow schema more strictly than working pages (working may accumulate patches that violate schema; canonical is the clean version).

If a working page's structure violates schema (e.g., section ordering, missing recommended sections that raw would support):
- If the fix is unambiguous (e.g., reorder sections to match schema), fix in canonical
- If the fix requires adding content (e.g., writing a Failure Modes section not present in working), DO NOT invent content. Preserve working's structure and note in the compaction log for schema-drift-audit or writer follow-up.

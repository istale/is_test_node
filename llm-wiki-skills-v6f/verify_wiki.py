...
def cmd_verify(args):
    ...
    # --- V1/V3/V6: wiki pages
    cited = set()  # (source_rel, hash) pairs cited anywhere in the wiki
    ...
    # --- V10: orphan expert statements — extracted but surfaced nowhere.
    # The wiki is a clue concordance; a clue that exists only in statements/
    # is invisible to a reader browsing the wiki. Typical omission-drift of a
    # small executor model: it extracted (or the script did) but forgot to
    # place the statement into any page.
    for src, stmts in stmt_index.items():
        for h, s in stmts.items():
            if (s["type"] == "verbatim" and s["speaker"] == "expert"
                    and (src, h) not in cited):
                rpt.add("MEDIUM", "V10-ORPHAN", f"{src}#{h}",
                        f"expert statement extracted but cited by no wiki page "
                        f"— clue not findable via the wiki "
                        f"({s['text'][:40].replace(chr(10), ' ')}...)")

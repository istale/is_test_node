...
### stmt 3f9a1c2b8d4e | environmental-monitoring | verbatim
...
3. Copy `tools/verify_wiki.py` from this skill into `$WIKI/tools/`
...
```bash
npm install -g obsidian-headless          # requires Node.js 22+
ob login --email <email>                  # prompts for password interactively —
                                          # NEVER pass --password on the command
                                          # line; it leaks into shell history and
                                          # the process table
ob sync-create-remote --name "LLM Wiki"
cd ~/wiki
ob sync-setup --vault "<vault-id>"
ob sync
ob sync --continuous                       # foreground; use systemd for background
```
